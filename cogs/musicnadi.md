    ...
    constructor(client) {
        ...
        this.riffy.on("trackStart", async (player, track) => {
            ...
            try {
                ...
                const selects = await this.buildSelectRows(player.guildId, track);
                const rows = this.buildControlRows(player.guildId, player);
...
_${track.info.author}_`,
    ...
    buildControlRows(guildId, player) {
        const isPaused = Boolean(player?.paused);
        const toggleEmoji = isPaused ? '▶️' : '⏸️';
        const row1 = new ActionRowBuilder().addComponents(
            new ButtonBuilder().setCustomId(`ctl_play_${guildId}`).setEmoji('▶').setStyle(ButtonStyle.Secondary),
            // previous track
            new ButtonBuilder().setCustomId(`ctl_prevtrack_${guildId}`).setEmoji('⏮️').setStyle(ButtonStyle.Secondary),
            new ButtonBuilder().setCustomId(`ctl_pause_${guildId}`).setEmoji(toggleEmoji).setStyle(ButtonStyle.Secondary),
            // next track
            new ButtonBuilder().setCustomId(`ctl_skip_${guildId}`).setEmoji('⏭️').setStyle(ButtonStyle.Secondary),
            // jump info helper
            new ButtonBuilder().setCustomId(`ctl_jump_${guildId}`).setEmoji('🔁').setStyle(ButtonStyle.Secondary),
        );
        const row2 = new ActionRowBuilder().addComponents(
            new ButtonBuilder().setCustomId(`ctl_voldown_${guildId}`).setEmoji('🔉').setStyle(ButtonStyle.Secondary),
            // quick slow mode toggle
            new ButtonBuilder().setCustomId(`ctl_qslow_${guildId}`).setEmoji('🐌').setStyle(ButtonStyle.Secondary),
            new ButtonBuilder().setCustomId(`ctl_like_${guildId}`).setEmoji('❤️').setStyle(ButtonStyle.Secondary),
            // quick fast mode toggle
            new ButtonBuilder().setCustomId(`ctl_qfast_${guildId}`).setEmoji('⚡').setStyle(ButtonStyle.Secondary),
            new ButtonBuilder().setCustomId(`ctl_volup_${guildId}`).setEmoji('🔊').setStyle(ButtonStyle.Secondary),
        );
        const row3 = new ActionRowBuilder().addComponents(
            new ButtonBuilder().setCustomId(`ctl_247_${guildId}`).setEmoji('🕐').setStyle(ButtonStyle.Secondary),
            new ButtonBuilder().setCustomId(`ctl_loop_${guildId}`).setEmoji('🔁').setStyle(ButtonStyle.Secondary),
            new ButtonBuilder().setCustomId(`ctl_clear_${guildId}`).setEmoji('🗑️').setStyle(ButtonStyle.Secondary),
            new ButtonBuilder().setCustomId(`ctl_filter_${guildId}`).setEmoji('🎛️').setStyle(ButtonStyle.Secondary),
            new ButtonBuilder().setCustomId(`ctl_stop_${guildId}`).setEmoji('⏹️').setStyle(ButtonStyle.Secondary),
        );
        return [row1, row2, row3];
    }

    async buildSelectRows(guildId, track) {
        const rows = [];
        try {
    ...
    buildQueuePageEmbed(player, page, perPage) {
        ...
        embed.setFooter({ text: `Page ${page + 1} of ${Math.max(1, Math.ceil(player.queue.size / perPage))}` });
        return embed;
    }
    ...
    async handleControlInteraction(interaction) {
        ...
        // Check DJ permissions
            ...
            if (!this.userIsDJ(member)) {
            await this.safeReply(interaction, 'DJ role required.');
                return;
            }
        ...
        // Update control panel if needed
            try {
                ...
                if (controlId && interaction.message?.id === controlId && player.current) {
                    ...
                    const rows = this.buildControlRows(guildId, player);
                    await interaction.message.edit({ embeds: [updated], components: rows });
                }
        } catch (updateError) {
